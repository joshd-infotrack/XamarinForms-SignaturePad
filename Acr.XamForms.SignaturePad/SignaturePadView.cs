...
namespace Acr.XamForms.SignaturePad {
    ...
    public class SignaturePadView : View {
        private Func<ImageFormatType, byte[]> getImageFunc;
        private Func<bool> clearFunc;
        ...
        public byte[] GetImage(ImageFormatType imageFormat) {
            return this.getImageFunc(imageFormat);
        }

        public bool Clear()
        {
            return this.clearFunc();
        }
        ...
        public void SetInternals(Func<ImageFormatType, byte[]> getImage, Func<IEnumerable<DrawPoint>> getPoints, 
            Action<IEnumerable<DrawPoint>> loadPoints,  Func<bool> isBlank, Func<bool> clear ) {
